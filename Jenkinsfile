pipeline {
	...
	// only runs on the 'master' branch     
       stage('Push to Artifactory - JFrog eproc-maven'){
	    ...
	    when {
		branch 'develop'    
  		environment name: 'GIT_URL', value: 'https://github.dxc.com/eProcurement-V2/catalogue-admin-service.git'
	    }	
            steps {
	...
	// send a build pipeline messages to MS Teams    
       ...
       stage('Verify Artifacts - JFrog Maven'){
            steps {
                   ...
                   sh "cp target/catalogue-admin-service-0.0.1-SNAPSHOT.jar catalogue-admin-service.jar"         
            }
            //steps
        }     
        // Docker build 
       stage('Build Docker Image - Local'){
            steps {
                   sh '''
                   docker login docker.dxc.com/eprocurement-docker -u svc-eprocurement-maven  -p Cw23@wg45
